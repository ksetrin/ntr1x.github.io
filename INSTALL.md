...
Данная схема развёртывания предназначена для использования в облачном режиме.
Используйте её, если вам нужна масштабируемость и производительность.
...
Инструкции написаны для `Debian GNU/Linux 8 (jessie)`. Некоторые компоненты не вляются строго необходимыми и могут быть заменены, другие компоненты нужны только при сборке платформы и не нужны для её работы, однако инструкции составлены в предположении, что используются именно эти компоненты.
...
Допускается дублирование компонент платформы. В Production-окружении нагрузка на все компоненты может быть сбалансирована посредством Nginx-инструкции `upstream`. Такая конфигурация уже специфична для установки.

Далее необходимо создать символическую ссылку на данный файл конфигурации в конфигурационном каталоге прокси-севера Nginx `sites-available` и перезапустить сервер Nginx:
...
Далее следует настроить компоненты Control Panel, Designer и Viewer.
В домашнем каталоге администратора системы следует разместить следующий исполняемый файл:
...
Следует также вручную создать каталоги:
...
Эта команда загрузит дистрибутив из Git-репозитария.
...
любые другие учётные данные, убедившись, что в конфигурационном файле указаны именно они.
...
Разместите в домашней папке администратора системы сценарий загрузки и установки:
...
Для того, чтобы клиентский модуль платформы взаимодействовал с данной установкой серверных модулей, необходимо внести изменения в файлы:
- `ntr1x-archery/config/default.js`
- `ntr1x-archery/config/development.js`
- `ntr1x-archery/config/production.js`

Достаточно лишь указать верное расположение серверных модулей.
После изменения этих файлов необходимо перезапустить клиентский модуль сценарием `build-com.example.archery`.

### Обновление и перезапуск

Для полного обновления и перезапуска платформы после настройки
требуется выполнить администратором системы 2 команды:
...
Во избежание конфликтов данные сценарии всегда должны выполняться с правами одного и того же пользователя системы.

## Режим разработчика

Данная схема развёртывания предназначена для использования в локальном окружении разработчика.
...
Для работы с платформой в режиме разработчика нет необходимости в
установке и настройке сервера Nginx. При этом компоненты Control Panel,
Designer и Viewer, запущенные локально, будут по-умолчанию взаимодействовать
с установками Storage и Source на demo-сервере платформы.

Таким образом, достаточно настроить сценарий сборки и запуска клиентской
части платформы (по приведённым в предыдущем разделе инструкциям).

Если требуется использовать локальную установку компонент Storage и Source,
потребуется также настроить сценарий сборки и запуска серверной части
платформы (по приведённым в предыдущем разделе инструкциям).

Однако потребуется изменить настройки в конфигурационных файлах
клиентских модулей. Данные настройки лежат в файлах:
- `ntr1x-archery/config/default.js`
- `ntr1x-archery/config/development.js`
- `ntr1x-archery/config/production.js`
